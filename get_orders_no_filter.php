...
echo "=== LẤY ĐƠN HÀNG KHÔNG CÓ FILTER ===\n\n";
...
try {
    ...
    // Test 1: Thử với filter thời gian rộng hơn (không có filter mặc định)
    echo "📝 Test 1: Với filter thời gian rộng (không có filter mặc định)\n";
    
    // Tạm thời comment out filter mặc định trong TikTokOrderService
    // Hoặc thử với filter thời gian rộng hơn
    $veryOldTime = strtotime('-10 years'); // 10 năm trước
    $result1 = $orderService->searchOrders($shop, ['create_time_ge' => $veryOldTime], 100);
    ...
    // Test 2: Thử với filter thời gian rộng nhất có thể
    echo "📝 Test 2: Với filter thời gian rộng nhất (20 năm)\n";
    $veryVeryOldTime = strtotime('-20 years'); // 20 năm trước
    $result2 = $orderService->searchOrders($shop, ['create_time_ge' => $veryVeryOldTime], 100);
    ...
    // Test 3: Thử với filter thời gian rộng nhất có thể (50 năm)
    echo "📝 Test 3: Với filter thời gian rộng nhất (50 năm)\n";
    $veryVeryVeryOldTime = strtotime('-50 years'); // 50 năm trước
    $result3 = $orderService->searchOrders($shop, ['create_time_ge' => $veryVeryVeryOldTime], 100);
    ...
    if ($result3['success']) {
        ...
        if (!empty($orderList3)) {
            ...
            echo "   📭 Không có đơn hàng nào trong 50 năm gần đây\n";
        }
    } else {
    ...
    // Test 4: Thử với filter thời gian rộng nhất có thể (100 năm)
    echo "📝 Test 4: Với filter thời gian rộng nhất (100 năm)\n";
    $veryVeryVeryVeryOldTime = strtotime('-100 years'); // 100 năm trước
    $result4 = $orderService->searchOrders($shop, ['create_time_ge' => $veryVeryVeryVeryOldTime], 100);
    ...
    if ($result4['success']) {
        ...
        if (!empty($orderList4)) {
            ...
            echo "   📭 Không có đơn hàng nào trong 100 năm gần đây\n";
        }
    } else {
    ...
    if ($totalOrders > 0) {
        ...
        echo "   Có thể:\n";
        echo "   - Shop này thực sự chưa có đơn hàng\n";
        echo "   - Có vấn đề với API hoặc shop\n";
        echo "   - Cần kiểm tra lại thông tin shop\n";
        echo "   - Có thể shop đang ở chế độ test/sandbox\n";
    }

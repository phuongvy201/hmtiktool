    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between mb-6">
                <div class="flex items-center">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-white mb-2">Chi tiết Backup</h1>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2">
                ...
                <div class="bg-gray-800 rounded-xl border border-gray-700 p-6 mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-400 mb-1">Tên file</label>
                        ...
                        <div>
                            ...
                            <p class="text-white">{{ $backup->description ?: 'Không có mô tả' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-400 mb-1">Kích thước</label>
                            <p class="text-white">{{ $backup->formatted_file_size }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-400 mb-1">Thời gian thực hiện</label>
                            <p class="text-white">{{ $backup->formatted_duration }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-400 mb-1">Số bảng</label>
                            <p class="text-white">{{ $backup->tables_count }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-400 mb-1">Số records</label>
                ...
                <div class="bg-gray-800 rounded-xl border border-gray-700 p-6 mb-6">
                    <div class="flex items-center mb-6">
                        ...
                        <h2 class="text-xl font-semibold text-white">Thông tin thời gian</h2>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-400 mb-1">Bắt đầu</label>
                            <p class="text-white">{{ $backup->started_at->format('d/m/Y H:i:s') }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-400 mb-1">Hoàn thành</label>
                            <p class="text-white">{{ $backup->completed_at ? $backup->completed_at->format('d/m/Y H:i:s') : 'Chưa hoàn thành' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-400 mb-1">Người tạo</label>
                            <p class="text-white">{{ $backup->creator?->name ?? 'System' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-400 mb-1">Tuổi file</label>
                            <p class="text-white">{{ $backup->file_age_in_days }} ngày</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gray-800 rounded-xl border border-gray-700 p-6 mb-6">
                    <div class="flex items-center mb-6">
                        ...
                        <h2 class="text-xl font-semibold text-white">Chi tiết kỹ thuật</h2>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-400 mb-1">Loại nén</label>
                            <p class="text-white">{{ ucfirst($backup->compression_type) }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-400 mb-1">Mã hóa</label>
                            <p class="text-white">{{ $backup->is_encrypted ? 'Có' : 'Không' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-400 mb-1">Đường dẫn file</label>
                            <p class="text-white text-sm break-all">{{ $backup->file_path ?: 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-400 mb-1">File tồn tại</label>
                            <p class="text-white">{{ $backup->fileExists() ? 'Có' : 'Không' }}</p>
                        </div>
                    </div>

                    @if($backup->excluded_tables && count($backup->excluded_tables) > 0)
                        <div class="mt-6">
                            <label class="block text-sm font-medium text-gray-400 mb-2">Bảng loại trừ</label>
                    ...
                    @if($backup->tables_list && count($backup->tables_list) > 0)
                        <div class="mt-6">
                            <label class="block text-sm font-medium text-gray-400 mb-2">Bảng được backup</label>
                            <div class="flex flex-wrap gap-2">
                ...
                @if($backup->error_message)
                    <div class="bg-red-500/10 border border-red-500/20 rounded-xl p-6">
                        <div class="flex items-center mb-4">
                            ...
                            <h2 class="text-xl font-semibold text-white">Thông tin lỗi</h2>
                        </div>
            ...
            <div class="lg:col-span-1">
                <!-- Actions -->
                <div class="bg-gray-800 rounded-xl border border-gray-700 p-6 mb-6">
                    <h3 class="text-lg font-semibold text-white mb-4">Thao tác</h3>
                    <div class="space-y-3">
                        @if($backup->isCompleted() && $backup->type === 'backup')
                            <a href="{{ route('backups.download', $backup) }}" 
                        ...
                        <form method="POST" action="{{ route('backups.destroy', $backup) }}" class="w-full">
                            ...
                            @method('DELETE')
                            <button type="submit" 
                                    onclick="return confirm('Bạn có chắc chắn muốn xóa backup này?')"
                ...
                <!-- Status Card -->
                <div class="bg-gray-800 rounded-xl border border-gray-700 p-6 mb-6">
                    <h3 class="text-lg font-semibold text-white mb-4">Trạng thái</h3>
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between">
                            <span class="text-gray-400">File tồn tại:</span>
                            <span class="text-{{ $backup->fileExists() ? 'green' : 'red' }}-400">
                                {{ $backup->fileExists() ? 'Có' : 'Không' }}
                            </span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-400">Mã hóa:</span>
                            <span class="text-{{ $backup->is_encrypted ? 'green' : 'gray' }}-400">
                                {{ $backup->is_encrypted ? 'Có' : 'Không' }}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gray-800 rounded-xl border border-gray-700 p-6">
                    <h3 class="text-lg font-semibold text-white mb-4">Thông tin file</h3>
                    <div class="space-y-3">
                        <div class="flex justify-between">
                        ...
                        <div class="flex justify-between">
                            <span class="text-gray-400">Tuổi file:</span>
                            <span class="text-white">{{ $backup->file_age_in_days }} ngày</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-400">Thời gian thực hiện:</span>
                            <span class="text-white">{{ $backup->formatted_duration }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-400">Số bảng:</span>
                            <span class="text-white">{{ $backup->tables_count }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-400">Số records:</span>
...
<div id="restoreModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-gray-800 rounded-xl border border-gray-700 p-6 w-full max-w-md">
            <h3 class="text-lg font-semibold text-white mb-4">Khôi phục từ Backup</h3>
            <p class="text-gray-400 mb-6">Cảnh báo: Hành động này sẽ ghi đè dữ liệu hiện tại!</p>
            
            <form method="post" action="{{ route('backups.restore', $backup) }}" class="space-y-4">
                @csrf
                
                <div>
                    <label for="restore_description" class="block text-sm font-medium text-gray-300 mb-2">Mô tả</label>
                    <input type="text" id="restore_description" name="description" 
                           value="Restore từ backup: {{ $backup->filename }}"

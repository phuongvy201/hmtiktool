    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="mb-8">
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h1 class="text-3xl font-bold text-white mb-2">Quản lý Team TikTok Shop</h1>
                    <p class="text-gray-400">Quản lý các team TikTok Shop và tổ chức trong hệ thống</p>
                </div>
                @can('create-teams')
            ...
            <form method="GET" action="{{ route('teams.index') }}" class="flex flex-wrap gap-4">
                <div class="flex-1 min-w-64">
                    <input type="text" name="search" value="{{ request('search') }}" 
                           placeholder="Tìm kiếm theo tên team TikTok Shop hoặc mô tả..." 
                ...
                <div class="flex gap-2">
                    ...
                    <select name="member_count" class="bg-gray-700 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-blue-500">
                        <option value="">Tất cả số thành viên</option>
                        <option value="0" {{ request('member_count') == '0' ? 'selected' : '' }}>Không có thành viên</option>
                        <option value="1-5" {{ request('member_count') == '1-5' ? 'selected' : '' }}>1-5 thành viên</option>
                        <option value="6-10" {{ request('member_count') == '6-10' ? 'selected' : '' }}>6-10 thành viên</option>
                        <option value="10+" {{ request('member_count') == '10+' ? 'selected' : '' }}>10+ thành viên</option>
                    </select>
                ...
                <table class="w-full">
                    <thead class="bg-gray-700">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Mô tả</th>
                        ...
                        <tr class="hover:bg-gray-700 transition-colors duration-200">
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-gray-400 text-sm">{{ $team->users->count() }} thành viên</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                ...
                                @else
                                    <span class="text-gray-500 text-sm">Không có mô tả</span>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                @if($team->users->count() > 0)
                                    <div class="flex items-center">
                                        <span class="text-gray-300 mr-2">{{ $team->users->count() }} thành viên</span>
                                ...
                                @else
                                    <span class="text-gray-500 text-sm">Không có thành viên</span>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-2">
                                    @can('view-teams')
                                    <a href="{{ route('teams.show', $team) }}" class="text-blue-400 hover:text-blue-300 transition-colors duration-200" title="Xem chi tiết">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @endcan
                                    @can('edit-teams')
                                    <a href="{{ route('teams.edit', $team) }}" class="text-yellow-400 hover:text-yellow-300 transition-colors duration-200" title="Chỉnh sửa">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-8 text-center text-gray-400">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-lg font-medium">Không có team nào</p>
                                    <p class="text-sm">Bắt đầu bằng cách tạo team mới</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse

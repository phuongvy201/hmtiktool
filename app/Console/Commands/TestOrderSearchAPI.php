...
class TestOrderSearchAPI extends Command
{
    protected $signature = 'test:order-search-api {order_id}';
    protected $description = 'Test POST /order/202309/orders/search API to find order by ID';

    public function handle()
    {
        $orderId = $this->argument('order_id');

        $this->info("Testing POST /order/202309/orders/search API for order ID: {$orderId}");
